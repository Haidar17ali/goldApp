...
@section('title', 'Ubah Purchase Order ' . $type)

@section('content_header')
    <h1>Ubah Purchase Order {{ $type }}</h1>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <div class="badge badge-primary float-right">Ubah Purchase Order {{ $type }}</div>
        </div>
    </div>
    <form action="{{ route('purchase-order.update', ['type' => $type, 'id' => $po->id]) }}" method="POST" id="formRP">
        @csrf
        @method('patch')
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        ...
                        @if ($type == 'Sengon')
                                        ...
                                        @if (count($supplier_types))
                                            @foreach ($supplier_types as $supplier_type)
                                                <option {{ $supplier_type == $po->supplier_type ? 'selected' : '' }}
                                                    value="{{ $supplier_type }}">{{ $supplier_type }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                </div><label for="wood_type" class="col-sm-2 col-form-label">Jenis Kayu</label>
                                <div class="col-sm-4">
                                    <select class="form-control" name="wood_type" id="wood_type">
                                        ...
                                        @if (count($wood_types))
                                            @foreach ($wood_types as $wood_type)
                                                <option {{ $po->wood_type == $wood_type ? 'selected' : '' }}
                                                    value="{{ $wood_type }}">{{ $wood_type }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                        ...
                        <div class="float-right mt-3">
                            <a href="{{ route('purchase-order.index', $type) }}"
        ...
        $(document).ready(function() {
                    ...
                    function getLocalStorage() {
                        ...
                        if (savedData) {
                            ...
                            const initialData = {!! $po->details != null ? json_encode($po->details) : [] !!}
