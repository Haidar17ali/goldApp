    ...
    <div class="shadow-lg card">
        <div class="card-body">
            ...
            <form id="transactionForm" method="POST" action="{{ route('penjualan.online.simpan', ['id' => $id]) }}"
                ...
                {{-- ================= PAYMENT ================= --}}
                @include('components.marketplace-payment', [
...
@section('js')
    ...
    <script>
        ...
        function calculateMarketplacePayment() {

            const buyer =
                parseFloat($('#buyerPaid').val()) || 0;

            const received =
                parseFloat($('#receivedAmount').val()) || 0;

            // biaya marketplace
            let fee = buyer - received;

            if (fee < 0)
                fee = 0;

            // kembalian
            let change = received - invoiceTotal;
        ...
        $('#buyerPaid,#receivedAmount').on(
            'keyup change',
            calculateMarketplacePayment
        );

        document.addEventListener('grandTotalChanged', function(e) {

            invoiceTotal = parseFloat(e.detail.total) || 0;
            ...
            <form id="transactionForm" method="POST"
                ...
                <div class="mb-4 row">
                    ...
                    <div class="col-md-4 position-relative" x-data="customerInput({
                        ...
                        <!-- DROPDOWN -->
                        <div class="shadow list-group position-absolute w-100" x-show="show"
                            @click.outside="show = false" style="z-index:1050">
                ...
                <div x-data="barcodeInput()" class="mb-4 position-relative">
                    ...
                    <div class="table-responsive">
                        ...
                        <div class="mb-2 col-md-12">
                            <label class="fw-semibold">Harga Manik</label>
                            <input type="number" name="manik_price" id="hargaManik"
                                class="form-control form-control-lg"
                                value="{{ old('manik_price', $transaction->manik_price) }}" placeholder="30000">
                        </div>
