...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <div class="badge badge-primary float-right">Ubah LPB</div>
        </div>
    </div>
    <form action="{{ route('lpb.update', $lpb->id) }}" method="POST" id="formRP">
        @csrf
        @method('patch')
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="number" class="form-control" id="no_kitir" name="no_kitir"
                                    value="{{ old('no_kitir', $lpb->no_kitir) }}">
                                ...
                                <select class="form-control" name="supplier_id" id="supplier_id">
                                    ...
                                    @if (count($suppliers))
                                        @foreach ($suppliers as $supplier)
                                            <option {{ $lpb->supplier_id == $supplier->id ? 'selected' : '' }}
                                ...
                                <select class="form-control" name="npwp_id" id="npwp_id">
                                    ...
                                    @if (count($npwps))
                                        @foreach ($npwps as $npwp)
                                            <option {{ $lpb->npwp_id == $npwp->id ? 'selected' : '' }}
                                                value="{{ $npwp->id }}">{{ $npwp->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <label for="nota_conversion" class="col-sm-2 col-form-label">Total Konversi</label>
                            <div class="col-sm-4">
                                <input type="number" readonly class="form-control" id="nota_conversion"
                                    name="nota_conversion" value="{{ old('nota_conversion', $lpb->nota_conversion) }}">
    ...
    <script>
        $(document).ready(function() {
                    ...
                    function getLocalStorage() {
                        const savedData = localStorage.getItem('editLpb');
