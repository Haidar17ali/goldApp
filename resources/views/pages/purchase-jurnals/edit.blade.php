...
@section('title', 'Ubah Purchase-Jurnal ')

@section('content_header')
    <h1>Ubah Purchase-Jurnal</h1>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <div class="badge badge-primary float-right">Ubah Purchase-Jurnal</div>
        </div>
    </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            const selectedLpbs = @json($lpbIds).map(String);
            const selectedLpbData = @json($lpbs);
            let supplierMap = {}; // Global
            // ajax select data
            let debounceTimer;

            // Inisialisasi deletedLPBs dari localStorage atau array kosong
            let deletedLPBs = new Set(JSON.parse(localStorage.getItem('editDeletedLPBs')) || []);
            ...
            function restoreSelectedLPBs() {
                selectedLPBs = new Set(JSON.parse(localStorage.getItem('editSelectedLPBs')) || []);
                selectedLPBData = JSON.parse(localStorage.getItem('editSelectedLPBData')) || [];
                deletedLPBs = new Set(JSON.parse(localStorage.getItem('editDeletedLPBs')) || []); // Tambah ini
            ...
            $(document).on('click', '.select-btn', function() {
                ...
                if (existingLPB) {
                    let isFromBackenda = selectedLPBs.has(lpbId);

                    if (isFromBackenda) {
                        // Tambahkan ID ke deletedLPBs
                        deletedLPBs.add(lpbId);
                        localStorage.setItem('editDeletedLPBs', JSON.stringify(Array.from(deletedLPBs)));
                    }
                    ...
                    // Hapus hanya ID yang sesuai dari deletedLPBs
                    deletedLPBs.delete(lpbId);

                    // Simpan perubahan ke localStorage (opsional, tergantung kebutuhan)
                    localStorage.setItem('editDeletedLPBs', JSON.stringify(Array.from(deletedLPBs)));
                }

                localStorage.setItem('editSelectedLPBs', JSON.stringify(Array.from(selectedLPBs)));
                localStorage.setItem('editSelectedLPBData', JSON.stringify(selectedLPBData));
            ...
            $(document).on('click', '.remove-lpb-btn', function() {
                ...
                // Cek apakah LPB yang dihapus berasal dari backend (termasuk saat edit)
                let isFromBackend = selectedLPBs.has(lpbId);

                if (isFromBackend) {
                    deletedLPBs.add(lpbId); // Simpan ID ke daftar yang akan dihapus di backend
                    localStorage.setItem('editDeletedLPBs', JSON.stringify(Array.from(deletedLPBs)));
                }
            ...
            function sendDataToLaravel(selectedLPBData, dpData, url) {
                $.ajax({
                    url: url,
                    type: 'PATCH',
                    dataType: 'json',
                    data: {
                        _token: "{{ csrf_token() }}",
                        lpbs: selectedLPBData, // data LPB terpilih
                        dp: dpData, // data DP
                        deleted_lpbs: Array.from(deletedLPBs) // LPB yang dihapus
                    },
                    success: function(response) {
                        // Bersihkan localStorage setelah berhasil dikirim
                        localStorage.removeItem('editSelectedLPBs');
                        localStorage.removeItem('editSelectedLPBData');
                        localStorage.removeItem('editDeletedLPBs'); // Hapus juga deleted LPBs
            ...
            $(document).on('click', '#kirimPembayaranBtn', function() {
                ...
                let id = "{{ $pj->id }}";
                let url = "{{ route('purchase-jurnal.update', ':id') }}"
                url = url.replace(':id', id);



                sendDataToLaravel(selectedLPBData, dpData, url);
            });
        });
    </script>
@stop
