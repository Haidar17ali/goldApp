...
<div class="shadow card">
    ...
    <div class="card-body">
        <form id="transactionForm" method="POST" action="{{ route('transaksi.simpan', ["type"=>$type,"purchaseType" =>$purchaseType]) }}">
            @csrf
            ...
            <div class="mb-4 row">
                <div class="col-md-4">
                    ...
                    <input type="text" name="invoice_number" id="invoice_number" 
                           class="form-control" value="{{ 'INV-' . strtoupper(Str::random(6)) }}" required>
                </div>
                <div class="col-md-4">
                    <label for="customer_name" class="form-label">Nama Customer</label>
                    <input type="text" name="customer_name" id="customer_name" 
                           class="form-control" placeholder="Masukkan nama customer" required>
                </div>
                <div class="col-md-4">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const variantMap = {};
    variants.forEach(v => { variantMap[v.label] = v; });
    ...
    function updateGrandTotal(hotInstance) {
        ...
        grandTotalElement.textContent = total.toLocaleString('id-ID', { minimumFractionDigits: 2 });
    }

    const hot = new Handsontable(container, {
        data: oldDetails.length ? oldDetails : [], // <<--- gunakan data lama
        colHeaders: ['Varian', 'Produk', 'Karat', 'Gram', 'Qty', 'Harga/gr', 'Subtotal'],
        columns: [
            { data: 'variant_label', type: 'autocomplete', source: variantNames, strict: false, allowInvalid: true },
            { data: 'product_name', type: 'text', readOnly: true },
            { data: 'karat_name', type: 'text', readOnly: true },
            { data: 'gram', type: 'numeric', numericFormat: { pattern: '0.000' }, readOnly: true },
            { data: 'qty', type: 'numeric' },
            { data: 'price_per_gram', type: 'numeric', numericFormat: { pattern: '0,0.00' } },
            { data: 'subtotal', type: 'numeric', readOnly: true, numericFormat: { pattern: '0,0.00' } },
        ],
        stretchH: 'all',
        rowHeaders: true,
        height: '260px',
            ...
            changes.forEach(([row, prop, oldValue, newValue]) => {
                if (prop === 'variant_label' && newValue) {
                    ...
                    if (v) {
                        ...
                        this.setDataAtRowProp(row, 'product_name', '');
                        this.setDataAtRowProp(row, 'karat_name', '');
                        this.setDataAtRowProp(row, 'gram', '');
                    }
                }
        ...
        afterRemoveRow() { updateGrandTotal(this); },
        afterCreateRow() { updateGrandTotal(this); },
    });
