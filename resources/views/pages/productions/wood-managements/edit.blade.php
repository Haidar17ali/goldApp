...
@section('title', 'Ubah Pengelolaan Kayu', $type)

@section('content_header')
    <h1>Ubah Pengelolaan Kayu {{ $type }}</h1>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <div class="badge badge-primary float-right">Ubah Pengelolaan Kayu {{ $type }}</div>
        </div>
    </div>
    <form action="{{ route('pengelolaan-kayu.update', ['type' => $type, 'id' => $woodManagement->id]) }}" method="POST"
        id="formRP">
        @csrf
        @method('patch')
        <div class="row">
                    ...
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="date" class="form-control" id="date" name="date"
                                    value="{{ old('date', $woodManagement->date) }}">
                                <span class="text-danger error-text" id="date_error"></span>
                            </div>
                            <label for="shift" class="col-sm-2 col-form-label">Grade</label>
                            <div class="col-sm-4">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" {{ $woodManagement->grade == 'Super' ? 'checked' : '' }}
                                        type="radio" name="grade" id="super" value="Super">
                                    <label class="form-check-label" for="super">Super</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" {{ $woodManagement->grade == 'Afkir' ? 'checked' : '' }}
                                        type="radio" name="grade" id="afkir" value="Afkir">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="number" class="form-control" id="no_kitir" name="no_kitir"
                                    value="{{ old('no_kitir', $woodManagement->no_kitir) }}">
                                ...
                                <select class="form-control" name="tally_id" id="tally_id">
                                    ...
                                    @if (count($rotariEmployees))
                                        @foreach ($rotariEmployees as $rotariEmployee)
                                            <option {{ $woodManagement->tally_id == $rotariEmployee->id ? 'selected' : '' }}
                                                value="{{ $rotariEmployee->id }}">{{ $rotariEmployee->alias_name }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                        ...
                        @if ($type != '5-feet')
                            <div class="form-group row">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" {{ $woodManagement->from == 130 ? 'checked' : '' }}
                                            type="radio" name="from" id="130" value="130">
                                        <label class="form-check-label" for="130">130</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" {{ $woodManagement->from == 260 ? 'checked' : '' }}
                                            type="radio" name="from" id="260" value="260">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" {{ $woodManagement->to == 130 ? 'checked' : '' }}
                                            type="radio" name="to" id="130" value="130">
                                        <label class="form-check-label" for="130">130</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" {{ $woodManagement->to == 260 ? 'checked' : '' }}
                                            type="radio" name="to" id="260" value="260">
                                        <label class="form-check-label" for="260">260</label>
                                    </div>
                                    <span class="text-danger error-text" id="shift_error"></span>
                                </div>
                        ...
                        <div class="float-right mt-3">
                            <a href="{{ route('pengelolaan-kayu.index', $type) }}"
        ...
        $(document).ready(function() {
                    ...
                    let columnType = [{
                            ...
                            source: function(query, callback) {
                                // dropdown hanya menampilkan no_kitir
                                callback(dropdownSource.map(item => item.no_kitir));
                            },
                            renderer: function(instance, td, row, col, prop, value, cellProperties) {
                                // cari item berdasarkan ID
                                const item = dropdownSource.find(option => option.id == value || option.no_kitir ==
                                    value);
                    ...
                    const hot = new Handsontable(container, {
                        ...
                        afterChange: function(changes, source) {
                            if (source === 'edit' || source === 'paste') {
                                ...
                                localStorage.setItem("edit{{ $type }}", JSON.stringify(data));
                            }
                        }
                    ...
                    function getLocalStorage() {
                        const savedData = localStorage.getItem("edit{{ $type }}");
                        ...
                        if (savedData != null) {
                            ...
                            localStorage.setItem("edit{{ $type }}", JSON.stringify(initialData));
                        }
                    };
