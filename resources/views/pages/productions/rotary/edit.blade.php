...
@section('title', 'Ubah Input Hasil', $type)

@section('content_header')
    <h1>Ubah Input Hasil {{ $type }}</h1>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <div class="badge badge-primary float-right">Ubah Input Hasil {{ $type }}</div>
        </div>
    </div>
    <form action="{{ route('rotari.update', ['type' => $type, 'id' => $rotari->id]) }}" method="POST" id="formRP">
        @csrf
        @method('patch')
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="date" class="form-control" id="date" name="date"
                                    value="{{ old('date', $rotari->date) }}">
                                <span class="text-danger error-text" id="date_error"></span>
                            </div>
                            <label for="shift" class="col-sm-2 col-form-label">Shift</label>
                            <div class="col-sm-4">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" {{ $rotari->shift == '1' ? 'checked' : '' }}
                                        type="radio" name="shift" id="shift1" value="1">
                                    <label class="form-check-label" for="shift1">1</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" {{ $rotari->shift == '2' ? 'checked' : '' }}
                                        type="radio" name="shift" id="shift2" value="2">
                                    <label class="form-check-label" for="shift2">2</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" {{ $rotari->shift == '3' ? 'checked' : '' }}
                                        type="radio" name="shift" id="shift3" value="3">
                            ...
                            <div class="col-sm-4">
                                <select class="form-control" multiple name="kitirs[]" id="no_lpb">
                                    ...
                                    @foreach ($kitirs as $kitir)
                                        @php
                                            $value = $kitir['source_type'] . '|' . $kitir['id'];
                                        @endphp
                                        <option value="{{ $value }}" @selected(in_array($value, $selectedSources))>
                                            {{ $kitir['no_kitir'] }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control" name="tally_id" id="tally_id">
                                    ...
                                    @if (count($rotariEmployees))
                                        @foreach ($rotariEmployees as $rotariEmployee)
                                            <option {{ $rotari->tally_id == $rotariEmployee->id ? 'selected' : '' }}
                        ...
                        <div class="float-right mt-3">
                            <a href="{{ route('rotari.index', $type) }}" class="btn btn-danger rounded-pill mr-2">Batal</a>
